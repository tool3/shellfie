import type { Template } from '../types';
import { macosTemplate } from './macos';
import { windowsTemplate } from './windows';
import { minimalTemplate } from './minimal';

export { macosTemplate } from './macos';
export { windowsTemplate } from './windows';
export { minimalTemplate } from './minimal';
export { createTemplate, defaultShell, defaultControlStyle } from './base';

export const templates = {
  macos: macosTemplate,
  windows: windowsTemplate,
  minimal: minimalTemplate,
} as const;

export type TemplateName = keyof typeof templates;

export const resolveTemplate = (template: TemplateName | Template | undefined): Template => {
  if (!template) return macosTemplate;
  if (typeof template === 'string') {
    const resolved = templates[template];
    if (!resolved) throw new Error(`Unknown template: ${template}`);
    return resolved;
  }
  return template;
};
